...
class NewsInteractor(private val newsRepository: INewsRepository) : NewsUseCase {
    override fun fetchHeadlineNews(): Flow<Resource<List<NewsModel>>> =
        newsRepository.fetchHeadlineNews()

    override fun fetchEverythingNews(): Flow<PagingData<NewsModel>> =
        newsRepository.fetchEverythingNews()

    override fun fetchNewsByQuery(query: String): Flow<Resource<List<NewsModel>>> =
        newsRepository.fetchNewsByQuery(query)

    override fun getAllFavoriteNews(): Flow<List<NewsModel>> = newsRepository.getAllFavoriteNews()

    override suspend fun setFavoriteNews(newsModel: NewsModel, state: Boolean) =
        newsRepository.setFavoriteNews(newsModel, state)

    override suspend fun isFavoriteNews(publishedAt: String): Boolean =
        newsRepository.isFavorite(publishedAt)
}